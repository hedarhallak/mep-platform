...
| GET  | `/api/admin/invoices` | COMPANY_ADMIN_UP | **(Section 119.6 — Phase 6-D-5 PR 2)** Paginated list of the tenant's invoices for the Billing page UI. Query params: `page` (default 1), `limit` (default 20, max 100), optional `type` (SUBSCRIPTION_RECURRING / TRAINING / CUSTOM_DEMAND / OTHER), optional `status` (DRAFT / QUOTE_SENT / APPROVED / PARTIAL_PAID / PAID / OVERDUE / VOID / REFUNDED). Response: `{ ok, invoices: [{ id, invoice_number, type, status, subtotal_cents, qst_cents, gst_cents, total_cents, amount_paid_cents, currency, issue_date, due_date, paid_date, quote_expires_at, customer_notes, created_at }], pagination: { page, limit, total, total_pages } }`. Always scoped by `req.user.company_id`. Excludes `internal_notes` + `pdf_url` + `approved_by` (SUPER_ADMIN-only fields). Sorted by `issue_date DESC, id DESC`. |
