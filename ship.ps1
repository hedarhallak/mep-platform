# ship.ps1 — one-command PR helper for Constrai (DECISIONS §139).
#
# Turns the 7-step PR ritual (branch → add → commit → push → pr create →
# pr merge) into ONE line. Stages ONLY the files you name (explicit paths —
# Pitfall #29 — so EOL churn / unrelated edits never sneak in).
#
...
Write-Host "`n✓ PR opened + auto-merge armed (squash)." -ForegroundColor Green
Write-Host "  Watch checks:  gh pr checks" -ForegroundColor Green
Write-Host "  After merge, clean up local:  git checkout $Base; git pull; git branch -D $Branch" -ForegroundColor DarkGray
