...
> Latest DECISIONS section: **Section 156** (July 5, 2026 — **BI dashboard shipped + THE ORPHAN-PROCESS INCIDENT**. Real `/bi` page + `GET /api/bi/overview` (workforce/hours/travel-CCQ/coverage), EN/FR, deployed + verified on prod. Root-caused why prod deploys were **silent no-ops for ~a month**: an orphaned node held :3000 with old code while pm2 crash-looped on EADDRINUSE (134k restarts) and health probes passed anyway — fixed + `deploy.sh` now verifies the port is owned by pm2's child. Also: Semgrep registry drift fixed (dependabot cooldown 7d + SHA-pinned actions, #463); Pitfall #56 = duplicate top-level i18n key shadows the earlier one (#461). Prior: §155 = Leave Management spec; §154 = Android on Google Play internal testing LIVE+verified. Recent chain: §152 = session checkpoint; §151 = test-coverage push (~69→~74% lines, thresholds ratcheted to 68/58/70/69, + standup duplicate-request bug fix w/ migration 039) PAUSED at ~74% (remainder = the 3 algorithmic files); §150 = customer Billing UI polish COMPLETE (status filter, §117.4 confirmation, invoice PDF download, invoice detail modal); §149 = mobile brought to web parity (Batch A-D + permission-driven dashboard + EN/FR parity test + store/api tests).
> Active program: **None mid-flight as of June 22 — Hedar's pick.** Candidates: (1) **Android follow-ups** — Play internal-testing is LIVE+verified (§154); remaining: device QA (back button / image picker / date pickers / safe-area), `eas submit` service-account JSON, production track (needs 12-tester × 14-day closed test); (2) **triage 4 Dependabot PRs (#452–455) + WIP feature #408** (only open PRs after the §154 branch cleanup); (3) the **sales deck** improvements (FR+EN, 18 slides, on Desktop — "acceptable in principle, needs work"); (4) resume **coverage → 80%** (§151.9: daily_dispatch / assignments / auto_assign, fresh session); (5) **PENDING PROD DEPLOY** — §150 + §151.3 are merged to main but NOT on prod (see below). Deferred: mobile crew deploy, web i18n Tier 3, Android FCM push.
...
> **Prod deploy status: UP TO DATE as of July 5, 2026** (§156 deploy — includes §150 billing UI, §151.3 standup fix, §156 BI dashboard + i18n fix). ⚠️ Note from the §156 incident: deploys between ~May 26 and June 25 were silent no-ops (orphan process on :3000); `deploy.sh` now hard-fails if the port isn't owned by pm2's child. Migration 039 was applied during the June 25 deploy.
