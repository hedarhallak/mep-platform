...
> Latest DECISIONS section: **Section 90 / Piece 90-D** (Phase 5 first real admin screen — All Companies list with search + sort, May 10, 2026). Section 85 = multi-tenant architecture (Model C single domain). Section 86 = Phase 1 Cloudflare + Origin SSL execution log. Section 87 = Phase 3 email login execution log. Sections 88–89 = Phase 4 (PostgreSQL RLS) — Stage 1 permissive (012), Stage 2 req.db migration of 22/22 routes, Stage 3 strict (013). Section 90 = Phase 5 architecture + execution (90-A done, 90-B done, 90-C done, 90-D done, 90-E next). Phases 1 + 2 + 3 + 4 done. Phase 5 in progress.
> Active program: **Multi-Tenant Migration (Section 85, Phases 1-8)** — Phase 1 (Cloudflare + Origin SSL) ✅ DONE, Phase 2 (Tenant Resolver) ✅ no-op after Model C pivot, Phase 3 (email-based login + DB schema) ✅ DONE and deployed, **Phase 4 (PostgreSQL Row-Level Security) ✅ DONE** — all 20 tenant tables under strict policies on prod, all authenticated routes on req.db. **Phase 5 (SUPER_ADMIN portal split)** — architecture in Section 90 (subdomain A + Vite multi-entry B2 + vhost backend split C2). **90-A ✅ DONE** (admin.constrai.ca subdomain live), **90-B ✅ DONE** (Express vhost split), **90-C ✅ DONE** (Vite multi-entry), **90-D ✅ DONE** (first real screen — All Companies list with `/api/super/companies/overview` backend, search + sort frontend, 7 RTL tests). 90-E (auth flow validation across portals) is next. Total roadmap 90-A → 90-F = ~4 days; ~3 days used. The Section 84 UI smoke test is **paused** until all 8 phases ship. Email migration SendGrid → Resend (Resend account ready, code change pending) sits between Phase 5 and Phase 6.
