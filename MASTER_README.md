...
> Latest DECISIONS section: **Section 149** (June 20-21, 2026 — PROGRAM: Mobile app update, COMPLETE). Brought `mep-mobile` to feature parity with the web: §147 Submit Request + Batch A-D screens (Pending/PO, Expenses/Surplus, Tools/Crews, Project Staffing, Standup, Employees+invite, Suppliers+create, Projects+create, Crews CRUD), a permission-driven Dashboard (§149.3, mirrors web gating), complete EN/FR i18n with a permanent parity test (`mep-mobile/src/i18n/parity.test.ts`, 425=425), and removal of dead `TasksScreen.tsx`. Earlier program context: §147 = project-centric assignment redesign; §148 = data-driven professional permissions (roles catalog + dynamic matrix + per-company control). Pre-§147 the active program was Phase 6-D (billing) — see §116-118.
> Active program: **§149 (Mobile update) COMPLETE.** Candidate next tracks (Hedar to pick): Phase 6-D-5 (customer-facing Subscription/Billing UI — still pending per the September 2026 conference roadmap), mobile crew deploy/plan (deferred — web doesn't expose it either), or web i18n Tier 3. No track is mid-flight as of June 21.
