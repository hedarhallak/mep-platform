...
| Latest deployed to prod | **June 11 (session 6):** PR #366 (`lib/email.js` welcome-email login link `/login.html`→`/login`) + prod `.env` gained `APP_URL=https://app.constrai.ca` (was unset → emails linked to localhost). Backend-only deploy (`git pull` + `pm2 restart`; no migration, no frontend). Also: MEP's OWNER provisioned via admin portal (no code — operational). Prior (June 6, session 5): §135 audit diff + §136 snapshot (migration 027) + §137 session caps (migration 028) + §140 OWNER Slice 1 (migration 029). **Latest migration on prod = 031.** |
| Last merged to main | **PR #366** (email login-link fix). June 6: #341–#348. OPEN: Dependabot #297/#298/#299 only (parked, green — hygiene phase). |
