...
2. **Read these 4 files** (use the Read tool, NOT bash):
   ...
   - Latest section is **141** (faster CI). **140** = OWNER role (DECISION "superset + exclusive audit" + slice plan; **Slice 1 DONE+LIVE PR #347** — see §140.5 three CI-caught bugs, §140.6 merge-friction pitfall; **build Slice 2 next**). **139** = dev automation (ship.ps1 + CD + migrate backfill). **138** = KNOWN-GAPS REGISTER G1–G7 for the incoming reviewing engineers. **137** = server-side SUPER_ADMIN session caps (idle 1h/abs 8h on /refresh, migration 028, PR #343). **136** = assignment location-snapshot at creation (migration 027, PR #342). **135** = REAL project-edit audit diff (PR #341; the June-4 "§135/#339" was a phantom, never coded — Pitfall #66 again). 134 = program overview + Settings page. 133 = SA session hardening (3 client layers). 132 = anti-tamper DESIGN SPEC (OWNER role etc.).
3. **Echo this exact line** as the first line of your reply:
   ```
   (محادثة استكمال — قرأت HANDOFF.md + DECISIONS.md §138-141, prod stable، §135-137 + OWNER Slice 1 نازلين، الجاي OWNER Slice 2)
   ```
4. **The track order is AGREED ("all", June 6) — execute, don't re-ask (Pitfall #51).** Done this session: §135 audit diff, §136 allowance snapshot, §137 server session caps. **NEXT in order:**
   - **§132 OWNER role — Slice 1 DONE+LIVE (PR #347). NEXT = Slice 2** (DECISIONS §140.3): the **exclusive audit viewer** — new owner-only sensitive-edit audit viewer (the §135/§136 old→new diffs); switch the audit gate from `settings.permissions` to an OWNER-only `audit.view`; REMOVE audit access from COMPANY_ADMIN's role_permissions; frontend OWNER audit page. Then Slice 3 (SUPER_ADMIN provisioning endpoint for the OWNER account + cross-tenant parent audit). Open sub-decisions in §140.4. Security-critical RBAC → build carefully, one focused question at a time.
...
| Latest deployed to prod | **June 6 (session 5) — all LIVE:** §135 audit diff + §136 snapshot (migration 027) + §137 session caps (migration 028) + **§140 OWNER role Slice 1 (migration 029 — role + 60-perm superset + app_users_role_check now allows OWNER + canAssignRole guard).** Final deploy ran `git pull` + `psql -f migrations/029` + restart; verified `grep -c "old_values: before.rows" routes/projects.js`=1 (§135 truly live now) + OWNER role row present. **§135 had NOT been on prod until this closeout** (PR #341 sat unmerged-behind for hours — §140.6). |
| Last merged to main | **PR #347** (OWNER Slice 1). Today merged: #341,#342,#343,#344,#345,#346,#347,#348. OPEN: Dependabot #297/#298/#299 only (parked, green — hygiene phase). |
...
| Prod env / ops | `INVOICE_EMAIL_ENABLED=true`, `TRIAL_WARN_DAYS=3`, **6 `DO_SPACES_*` vars (Spaces ACTIVE: `constrai-tenant-assets` TOR1+CDN)** in `/var/www/mep/.env`. Chrome libs for puppeteer. **Latest migration = 029** (027 = assignment location-snapshot columns, 028 = refresh_tokens session_started_at + last_activity_at, 029 = OWNER role + role_permissions copy + app_users_role_check allows OWNER — all applied on prod June 6; 024-026 earlier). New env (optional, have defaults): `SA_IDLE_MAX_MIN=60`, `SA_SESSION_ABS_MAX_MIN=480` for §137 caps. **schema_migrations backfill (§139) still NOT run on prod** — until then keep applying migrations manually via `sudo -u postgres psql -f` (not `npm run migrate`). `git config --global core.editor notepad` on Hedar's machine (vim trap closed). |
