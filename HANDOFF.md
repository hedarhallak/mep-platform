...
- **✅ DONE — §133 SUPER_ADMIN ephemeral session cookies + idle auto-logout:** (133.1-3) browser close now ends the SA session and forces login+TOTP on re-entry (`lib/cookie_options.js` `isEphemeralSessionRole` + `{ephemeral}`; `routes/auth.js` at login/TOTP/refresh; tenant cookies unchanged). VERIFIED in DevTools (access+refresh = `Expires: Session`). (133.4) `AdminIdleGuard.jsx` — 15-min idle auto-logout for the open-but-unattended browser case (frontend-only, mounted in AdminApp; `?reason=idle` banner on AdminLogin). **Server-side idle + absolute-session-cap enforcement = the remaining robust layer, left for the Security phase (§132/§133).**
