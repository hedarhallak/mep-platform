...
> Last updated: **June 20, 2026 (session 10)** — **§148 COMPLETE (Phases 1–5) + LIVE-VERIFIED end-to-end; §147 fully closed.** Highlights, newest first:
> • **§148 LIVE-VERIFICATION (June 20) — 5 fixes found by Hedar testing prod, all shipped + LIVE:** (a) **§148.17/.18** the User Management role dropdown AND the role-change backend (`PATCH /users/:id/role`) were each hardcoded to ~5 roles — Foreman/Journeyman/Apprentice/Driver + all 18 Phase-4 roles were unselectable / rejected `INVALID_ROLE`; both made data-driven from the `roles` catalog (`GET /permissions/roles` widened to `canAny(permissions, user_management)`). (b) **§148.19 — CRITICAL: the ENTIRE Phase 3b (§148.13) was silently absent from main** — `routes/permissions.js` had the original global-only matrix/PUT/reset/my-permissions, the frontend Reset-gating was SUPER_ADMIN/IT_ADMIN-only, and the per-company test was gone. PR #408 had shipped GREEN **without the code because its guarding test was lost in the same working-tree race** (§4.6 Cowork sync lag) — meaning a COMPANY_ADMIN editing the matrix was writing GLOBAL role_permissions (cross-tenant). Fully restored (#416). (c) **§148.20** added a per-user "Reset to default" button (DELETE /permissions/user/:id) distinct from the matrix role-reset. **LESSON (now a saved memory + §148.19): after every ship, `grep` for 2-3 code markers on merged main — a green CI does NOT prove the code landed if its test was lost too.** Prod deployed through #417.
