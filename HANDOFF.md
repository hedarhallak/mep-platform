...
| Latest deployed to prod | **June 11 (session 6), all LIVE backend-only:** PR #371 (pre-tenant invite/activate authPool RLS fix §142.5), PR #370 (Hub send RLS GUC fix), PR #369 (TOTP rate-limit), PR #368 (material pdf-data cross-tenant fix + **migration 032** `purchase_orders.po_number ADD COLUMN IF NOT EXISTS` — applied on prod inline), PR #366 (welcome-email login link `/login.html`→`/login`) + prod `.env` gained `APP_URL=https://app.constrai.ca`. Also MEP's OWNER provisioned (operational). Prior (June 6): §135–§137 (migrations 027/028) + §140 OWNER Slice 1 (029). **Latest migration on prod = 032.** |
| Last merged to main | **PR #371** (pre-tenant authPool RLS fix). Session 6: #366, #367 (docs), #368, #369, #370, #371. June 6: #341–#348. OPEN: Dependabot #297/#298/#299 only (parked, green — hygiene phase). |
