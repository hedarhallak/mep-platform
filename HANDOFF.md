...
4. **The track order is AGREED ("all", June 6) — execute, don't re-ask (Pitfall #51).** Done this session: §135 audit diff, §136 allowance snapshot, §137 server session caps. **NEXT in order:**
   - **§132 OWNER role (NEXT)** — Constrai-provisioned per tenant, sole audit viewer, separation-of-duties above the technical COMPANY_ADMIN; resolves the §129.9 approver-default question. Big architectural build → **one focused question at a time (§8.9 / rule #9)**. grep first (Pitfall #62): `user_permissions` table + `middleware/permissions.js can()` already honor per-user grants; there is NO grant UI/endpoint and NO OWNER role yet.
