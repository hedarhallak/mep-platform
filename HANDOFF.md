...
4. **The track order is AGREED ("all", June 6) — execute, don't re-ask (Pitfall #51).** Done this session: §135 audit diff, §136 allowance snapshot, §137 server session caps. **NEXT in order:**
   - **§132 OWNER role — Slice 1 + 2 + 3a + 3c COMPLETE + LIVE (the prevention+detection stack is functionally whole).** Slice 3c (PR #360) = "Provision OWNER" modal in admin `CompaniesList` (per-company "Owner →" → `POST /super/companies/:id/owner`). See §140.7. **NEXT operational: provision MEP's OWNER** from `admin.constrai.ca` → `/owner-audit` becomes visible (anti-tamper demoable for the conference). **NEXT code (optional/theoretical): Slice 3b** = cross-tenant parent-level audit copy + SUPER_ADMIN global view (§132.6 layer 6) — architectural, build fresh. §140.4 open sub-decisions.
