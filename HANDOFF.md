...
3. **Echo this exact line** as the first line of your reply:
   ```
   (محادثة استكمال — قرأت HANDOFF.md + DECISIONS.md §138-141، prod stable، §135-137 + OWNER Slice 1+2 backend نازلين، الجاي OWNER Slice 2c الواجهة)
   ```
4. **The track order is AGREED ("all", June 6) — execute, don't re-ask (Pitfall #51).** Done this session: §135 audit diff, §136 allowance snapshot, §137 server session caps. **NEXT in order:**
   - **§132 OWNER role — Slice 1 + Slice 2 (backend) DONE+LIVE.** Slice 2a/2b (PRs #352/#353, migrations 030/031): `GET /api/permissions/owner-audit` (the §132.7 sensitive-edit fraud viewer, old→new) gated `audit.view`, now **OWNER-exclusive** on prod (revoked from COMPANY_ADMIN+IT_ADMIN). See §140.7. **NEXT = Slice 2c** (frontend OWNER audit page consuming `/owner-audit`; decide how to surface the existing `/permissions/audit` RBAC-change view to whom). **Then Slice 3** = SUPER_ADMIN provisioning endpoint for the OWNER account (§132.5) + cross-tenant parent audit. Open sub-decisions in §140.4. Security-critical RBAC → one focused question at a time.
