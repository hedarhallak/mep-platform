...
3. **Echo this exact line** as the first line of your reply:
   ```
   (محادثة استكمال — قرأت HANDOFF.md + DECISIONS.md §138-141، prod stable، §135-137 + OWNER Slice 1+2+3a نازلين، الجاي OWNER Slice 3b cross-tenant audit + زر التزويد)
   ```
4. **The track order is AGREED ("all", June 6) — execute, don't re-ask (Pitfall #51).** Done this session: §135 audit diff, §136 allowance snapshot, §137 server session caps. **NEXT in order:**
   - **§132 OWNER role — Slice 1 + Slice 2 + Slice 3a COMPLETE + LIVE.** Slice 3a (PR #358) = `POST /api/super/companies/:id/owner` (SUPER_ADMIN provisions OWNER: role=OWNER user + temp-PIN email, one-per-company). See §140.7. **No OWNER provisioned yet** — provision when ready (then `/owner-audit` becomes visible). **NEXT = Slice 3b** (cross-tenant parent-level audit copy + SUPER_ADMIN global view, §132.6 layer 6) + **Slice 3c** ("Provision OWNER" button in admin CompaniesList instead of curl). Then §132 is whole. §140.4 open sub-decisions. Security-critical → one focused question at a time.
