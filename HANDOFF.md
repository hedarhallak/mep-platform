...
> Last updated: **June 19, 2026 (session 8)** — **§147 finished end-to-end + trade-scoping + started §148 professional-permissions program.** Highlights, newest first:
> • **§148 — PROFESSIONAL PERMISSIONS PROGRAM (the new big track, multi-session).** Long design discussion (decisions in DECISIONS §148.1). **LOCKED: roles = a FIXED rich catalog (NOT user-addable); what a role sees = PERMISSIONS not hierarchy; permissions must be controllable PER COMPANY** (layered resolution: `user_permissions` ▸ `company_role_permissions` (NEW, future) ▸ `role_permissions` global default). **Phase 1 (data-driven roles) = COMPLETE + LIVE:** migration 035 added `rank`+`category` to `roles` and upserted the 14 canonical (PR #396); migration 036 replaced the `app_users_role_check` CHECK with an **FK to `roles(role_key)`** so **adding a role is now a pure INSERT** (PR #397, verified safe vs prod + CI). `GET /api/permissions/roles` serves the catalog. **NEXT = Phase 2 (dynamic matrix):** PermissionsPage today hardcodes 12 modules × 5 actions, so audit/bi/hub/tasks + many actions + the `materials` module are INVISIBLE while dead `workforce_planner` shows — render ALL real permission codes from the catalog instead. Also fix the UX trap: the matrix opens on the viewer's OWN (rank-locked) role so it LOOKS read-only (§134) — default to the first editable role. Then Phase 3 (per-company layer), Phase 4 (expand catalog ~24 roles, §148.2), Phase 5 (per-user overrides UI). **The permission matrix IS already editable** (toggle per role + PUT `/permissions/role/:role`) — for roles ranked BELOW you; Hedar believed it wasn't, due to the open-on-own-locked-role trap.
...
> • **DEPLOY STATE (June 19, current):** prod is fully deployed at **#397** — §147 Phase 0/1/2 + trade-scoping (#395) + permissions Phase 1 (#396/#397) all LIVE. **Latest migration ON PROD = 036** (034 project_labor_requirements, 035 roles rank/category, 036 app_users role FK — all applied). Working tree clean on main, no loose ends. (Backend-only PRs deploy via `git pull` + `pm2 restart`; migration PRs run `sudo -u postgres psql -f migrations/NNN.sql` first, Pitfall #46; frontend PRs run `bash scripts/deploy.sh`, Pitfall #41.)
