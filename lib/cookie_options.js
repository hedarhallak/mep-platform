...
// Section 133 (June 4, 2026): high-privilege portals get EPHEMERAL (session)
// cookies — no maxAge, so the browser drops them the moment it closes. This
// forces a full re-login + TOTP on every new browser session for these roles
// (the persistent 7-day refresh cookie was letting a closed-then-reopened
// browser resume a SUPER_ADMIN session for up to a week WITHOUT re-prompting
// 2FA). Tenant roles keep the persistent ("remember me") cookies unchanged.
const EPHEMERAL_SESSION_ROLES = new Set(['SUPER_ADMIN']);

function isEphemeralSessionRole(role) {
  return EPHEMERAL_SESSION_ROLES.has(String(role || '').toUpperCase());
}
...
// `ephemeral: true` omits maxAge entirely → a SESSION cookie (dies on
// browser close). Used for high-privilege roles (see EPHEMERAL_SESSION_ROLES).
function accessTokenCookieOptions(req, { ephemeral = false } = {}) {
  const opts = baseCookieOptions(req);
  if (!ephemeral) opts.maxAge = ACCESS_TOKEN_MAX_AGE_MS;
  return opts;
}

function refreshTokenCookieOptions(req, { ephemeral = false } = {}) {
  const opts = baseCookieOptions(req);
  if (!ephemeral) opts.maxAge = REFRESH_TOKEN_MAX_AGE_MS;
  return opts;
}
