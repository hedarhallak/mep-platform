...
async function sendPurchaseOrder({
  ...
  <style>
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    <div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:32px;padding-bottom:24px;border-bottom:2px solid #4f46e5">
      <div>
        <div style="font-size:22px;font-weight:800;color:#4f46e5">${companyName || ''}</div>
        ${companyAddress ? `<div style="font-size:12px;color:#64748b;margin-top:4px">📍 ${companyAddress}</div>` : ''}
        ${companyPhone ? `<div style="font-size:12px;color:#64748b">📞 ${companyPhone}</div>` : ''}
      </div>
      <div style="text-align:right">
        <div style="font-size:20px;font-weight:800;color:#1e293b">Purchase Order</div>
        <div style="font-size:13px;color:#64748b;margin-top:4px">Ref: <strong>${ref}</strong></div>
    ...
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:16px;margin-bottom:20px">
      <div style="background:#f8fafc;border:1px solid #e2e8f0;border-radius:8px;padding:14px">
        ...
        <div style="font-size:14px;font-weight:700;color:#1e293b">${projectCode}${projectName ? ' — ' + projectName : ''}</div>
        ${siteAddress ? `<div style="font-size:12px;color:#64748b;margin-top:4px">📍 ${siteAddress}</div>` : ''}
      </div>
      <div style="background:#f8fafc;border:1px solid #e2e8f0;border-radius:8px;padding:14px">
        ...
        <div style="font-size:14px;font-weight:700;color:#1e293b">${foremanName || ''}</div>
        ${foremanPhone ? `<div style="font-size:12px;color:#64748b;margin-top:4px">📞 ${foremanPhone}</div>` : ''}
      </div>
    </div>
    ...
    <div style="${isProcurement ? 'background:#eff6ff;border:1px solid #bfdbfe' : 'background:#f8fafc;border:1px solid #e2e8f0'};border-radius:8px;padding:14px;margin-bottom:20px">
      ...
      <div style="font-size:14px;font-weight:700;color:#1e293b">${recipientLabel}</div>
    </div>
    ...
    ${note ? `<div style="background:#fffbeb;border:1px solid #fde68a;border-radius:8px;padding:14px;margin-bottom:24px">
      ...
      <div style="font-size:13px;color:#78350f">${note}</div>
    </div>` : ''}

    <div style="border-top:1px solid #e2e8f0;padding-top:16px;display:flex;justify-content:space-between">
      <div style="font-size:11px;color:#94a3b8">Generated by ${APP_NAME} · ${date}</div>
      <div style="font-size:11px;color:#94a3b8">${ref}</div>
    </div>
  </body></html>`;
  ...
  try {
    ...
    pdfBuffer = await page.pdf({ format: 'A4', margin: { top: '20mm', bottom: '20mm', left: '15mm', right: '15mm' }, printBackground: true });
    await page.close();
  } catch (err) {
  ...
  const itemText = (items || []).map((it, i) => `  ${i + 1}. ${it.item_name} — ${it.quantity} ${it.unit}`).join('\n');

  const emailHtml = `<!DOCTYPE html><html><head><meta charset="UTF-8"><style>${STYLES}</style></head><body>
  <div class="wrapper">
    <div class="header" style="background:#4f46e5;">
      <h1>📦 Purchase Order</h1>
      <p>${APP_NAME.toUpperCase()} · ${ref}</p>
    </div>
    <div class="body">
      <p class="greeting">Dear ${recipientLabel},</p>
      <p class="sub">Please find attached a material request from <strong>${companyName}</strong>.</p>
      <div class="info-box">
        <div class="row"><span class="label">Ref</span><span class="value">${ref}</span></div>
        <div class="row"><span class="label">Date</span><span class="value">${date}</span></div>
        <div class="row"><span class="label">Project</span><span class="value">${projectCode}${projectName ? ' — ' + projectName : ''}</span></div>
        ${siteAddress ? `<div class="row"><span class="label">Site</span><span class="value">${siteAddress}</span></div>` : ''}
        <div class="row"><span class="label">Requested By</span><span class="value">${foremanName}${foremanPhone ? ' · ' + foremanPhone : ''}</span></div>
      </div>
      ${note ? `<div class="notes-box">📝 ${note}</div>` : ''}
    </div>
    <div class="footer">${APP_NAME} — Ref: ${ref} · ${date}</div>
  </div>
</body></html>`;
...
module.exports = { sendEmail, sendAdminWelcome, sendAssignmentEmployee, sendAssignmentForeman, sendPurchaseOrder };
