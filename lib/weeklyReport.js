...
const sgMail = require('./email').getMailClient();
...
async function runWeeklyReports(pool) {
  ...
  // Section 98 cleanup: setApiKey removed. sgMail is the provider-agnostic
  // client from lib/email.js#getMailClient (Resend in prod).
...
// Pure helpers exported alongside runWeeklyReports so they can be unit-
// tested in isolation. Phase 67 (May 2026, Section 22 hardening): no
// behavioural change — just widening the test surface so the formatting
// + zone logic doesn't have to be exercised through the DB-heavy
// runWeeklyReports() path.
module.exports = {
  runWeeklyReports,
  ccqZone,
  fmtCAD,
