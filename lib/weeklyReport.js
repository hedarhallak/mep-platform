...
function buildEmployeeReportHtml({ employee, companyName, weekDays, attendanceMap, assignment }) {
  ...
  const dayRows = weekDays.map(date => {
    ...
    const isConfirmed = ["CONFIRMED", "ADJUSTED"].includes(status);
    ...
    }[status] || `<span style="background:#f1f5f9;color:#64748b;padding:2px 8px;border-radius:12px;font-size:11px;font-weight:700">${status}</span>`;
  ...
  // Unconfirmed warning
  const unconfirmedWarning = daysUnconfirmed > 0 ? `
  ...
  <div style="max-width:700px;margin:0 auto;padding:32px 16px">
    ...
    <div style="display:grid;grid-template-columns:1fr 1fr 1fr 1fr;gap:12px;margin-bottom:20px">
      ${[
        ...
        { label: "Overtime",       value: fmtHours(totalOvertime),     color: totalOvertime > 0 ? "#fffbeb" : "#f8fafc", txt: totalOvertime > 0 ? "#d97706" : "#94a3b8" },
        { label: "Total Hours",    value: fmtHours(totalRegular + totalOvertime), color: "#f0fdf4", txt: "#166534" },
      ].map(s => `
      ...
      <table style="width:100%;border-collapse:collapse">
        <thead>
          <tr style="background:#f8fafc">
            ${["Day","Check In","Check Out","Regular","Overtime","Status"].map(h =>
...
function buildForemanReminderHtml({ foreman, companyName, unconfirmedRecords, weekLabel }) {
  const rows = unconfirmedRecords.map(r => `
      ...
      <table style="width:100%;border-collapse:collapse">
        <thead>
          <tr style="background:#fef2f2">
            ${["Employee","Date","Check In → Out","Regular Hours"].map(h =>
...
async function runWeeklyReports(pool) {
  ...
  const { rows: companies } = await pool.query(
    `SELECT company_id, name FROM public.companies`
  );
    ...
    try {
      ...
      for (const foreman of foremen) {
        ...
        const myUnconfirmed = unconfirmed.filter(r => {
          const empAsgn = assignments.find(a =>
            a.employee_id === r.employee_id && a.project_id === foreman.project_id
          );
          return !!empAsgn;
        });
