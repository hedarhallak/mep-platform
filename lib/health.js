...
// `/var/lib/postgresql` is the data directory on the prod Droplet (Ubuntu).
// Override via DISK_CHECK_PATH for non-standard layouts or local dev.
...
// `/var/log/mep-backup.log` is appended to by scripts/backup/backup_db.sh on
// every run. Override via BACKUP_LOG_PATH for tests / dev.
const DEFAULT_BACKUP_LOG = process.env.BACKUP_LOG_PATH || '/var/log/mep-backup.log';
 ...
 */
async function checkDisk(diskPath = DEFAULT_DISK_PATH) {
  try {
 ...
 */
async function checkBackup(logPath = DEFAULT_BACKUP_LOG, now = new Date()) {
  let content;
  try {
