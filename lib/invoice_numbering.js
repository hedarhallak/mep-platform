 ...
 *
 * Tax rates (Section 116.6): stored as basis points in tax_rates table.
 *   - QC / QST: 9975 basis points = 9.975%
 *   - FEDERAL / GST: 500 basis points = 5%
 ...
 * Basis points: 1 bp = 0.01%. So qst_cents = subtotal_cents * bp / 10000.
...
function calculateTaxes(subtotalCents, rates) {
  ...
  const qst = Math.round((subtotal * Number(rates.qst_basis_points || 0)) / 10000);
  const gst = Math.round((subtotal * Number(rates.gst_basis_points || 0)) / 10000);
  return {
    qst_cents: qst,
    gst_cents: gst,
