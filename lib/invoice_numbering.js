...
async function generateInvoiceNumber(db, issueDate = new Date()) {
  ...
  // Take the MAX of the NUMERIC suffix, not a string ORDER BY. String ordering
  // breaks once the sequence crosses into more digits than the zero-pad width:
  // 'CONS-2026-9999' sorts ABOVE 'CONS-2026-10000' lexicographically ('9' > '1'),
  // so a string-max of 9999 → next 10000 → UNIQUE collision with the existing
  // 10000. Casting the trailing digits to int and MAX-ing them is order-correct
  // regardless of padding width. (Discovered by the Phase 6-D-7 monthly-invoice
  // smoke — Section 125.3.)
  ...
  const nextSeq = (Number(rows[0] && rows[0].max_seq) || 0) + 1;
