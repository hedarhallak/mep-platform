...
const mainNav = [
  ...
  },
  {
    ...
    permission: { module: 'assignments', action: 'view' },
  },
  {
    to: '/crews',
    icon: HardHat,
  ...
  },
  {
    ...
    icon: Target,
    labelKey: 'nav.projectStaffing',
    permission: { module: 'assignments', action: 'view' },
  },
  {
    to: '/assignments/submit',
    icon: Send,
    labelKey: 'nav.foremanRequest',
    permission: { module: 'assignments', action: 'create' },
  },
  {
    to: '/assignments/requests',
    icon: ClipboardCheck,
    labelKey: 'nav.pendingRequests',
    permission: { module: 'assignments', action: 'edit' },
  },
  {
    to: '/attendance',
    icon: CalendarCheck,
    labelKey: 'nav.attendance',
    permission: { module: 'attendance', action: 'view_self' },
  },
  {
    to: '/reports',
    icon: BarChart2,
  ...
  },
  {
    to: '/standup',
    icon: ClipboardList,
    labelKey: 'nav.standup',
    permission: { module: 'standup', action: 'manage' },
  },
  {
    to: '/task-request',
    icon: Send,
    labelKey: 'nav.taskRequest',
    permission: { module: 'hub', action: 'send_tasks' },
  },
  {
    to: '/material-request',
    icon: Package,
    labelKey: 'nav.materialRequest',
    permission: { module: 'materials', action: 'request_submit' },
  },
  {
    to: '/purchase-orders',
    icon: FileText,
  ...
  },
  {
    to: '/surplus',
    icon: Recycle,
    labelKey: 'nav.surplus',
    permission: { module: 'materials', action: 'surplus_view' },
  },
  {
    to: '/tools',
    icon: Wrench,
  ...
  },
  {
    ...
    permission: { module: 'expense_claims', action: 'submit' },
  },
  {
    to: '/owner-audit',
    icon: Shield,
    labelKey: 'nav.ownerAudit',
...
export default function AppLayout() {
  ...
  useEffect(() => {
    ...
    const fetchAttendance = async () => {
      try {
        ...
        const count = (r.data.records || []).filter(
          (rec) => rec.attendance_status === 'CHECKED_OUT'
        ).length;
  ...
  const canSeeReports = !permsLoading && (can('reports', 'view') || can('reports', 'view_self'));
  ...
  const canSeeSurplus =
    !permsLoading && (can('materials', 'surplus_view') || can('materials', 'surplus_declare'));
  const canSeeTools =
    !permsLoading && (can('materials', 'request_submit') || can('materials', 'surplus_view'));
  const canSeeExpenses =
    !permsLoading && (can('expense_claims', 'submit') || can('expense_claims', 'view'));
      ...
      <div className="flex h-screen overflow-hidden">
        ...
        <aside className="w-56 flex flex-col bg-[#0f172a] text-slate-300 flex-shrink-0">
          ...
          <div className="px-4 py-4 border-t border-slate-800">
            {/* In-app language toggle — switch FR/EN anywhere, not just at login. */}
            <div className="mb-3 flex justify-center">
              <LanguageSwitcher />
            </div>
            <div className="text-xs text-slate-500 mb-1 truncate">
              {user?.company_name || t('nav.companyFallback')}
            </div>
