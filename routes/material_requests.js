 ...
 *
 * Section 89-C/10: migrated from `pool.query` to `req.db.query`.
 * The `tenantDb` middleware (mounted in app.js) now wraps every request
 * in a per-request transaction with `app.company_id` set on the GUC, so
 * RLS policies enforce cross-tenant isolation automatically. The three
 * pre-existing manual `pool.connect()/BEGIN/COMMIT` blocks (POST
 * /requests, PATCH /:id/review, POST /returns) collapsed to plain
 * sequences of `req.db.query` calls — the middleware-level transaction
 * already gives us atomicity, and a nested BEGIN inside that tx is a
...
async function resolveEmployeeId(req) {
  ...
  if (userId) {
    const r = await req.db.query(`SELECT employee_id FROM public.app_users WHERE id = $1 LIMIT 1`, [
      userId,
    ]);
...
router.post('/requests', can('materials.request_submit'), async (req, res) => {
  try {
    ...
    // tenantDb wraps the whole request in one transaction, so we just
    // run the inserts in sequence on req.db. Any thrown error bubbles
    // up and tenantDb rolls back automatically.
...
router.patch('/requests/:id/review', can('hub.materials_merge_send'), async (req, res) => {
  try {
    ...
    // tenantDb already opened a transaction — run updates in sequence.
...
router.post('/returns', can('materials.surplus_declare'), async (req, res) => {
  try {
    ...
    // tenantDb already wraps this in a transaction.
    const {
      rows: [ret],
    } = await req.db.query(
