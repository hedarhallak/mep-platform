  ...
  try {
    ...
    if (!user || !pinOk) {
      await audit(pool, req, {
        action: ACTIONS.LOGIN_FAILED, entity_type: "user",
        entity_name: username, details: { reason: "INVALID_CREDENTIALS" },
      });
    ...
    await audit(pool, req, {
      action: ACTIONS.LOGIN_SUCCESS, entity_type: "user",
      entity_id: user.id, entity_name: user.username, details: { role },
    });
    ...
    return res.json({
      ...
      user: {
        ...
        employee_id: user.employee_id, company_id: user.company_id,
        role, must_change_pin: mustChangePin,
      },
    });
  } catch (err) {
...
router.post("/refresh", async (req, res) => {
  try {
    ...
    const payload = buildTokenPayload(
      { id: record.user_id, username: record.username, employee_id: record.employee_id, company_id: record.company_id },
      role, mustChangePin
    );
    ...
    return res.json({
      ...
      user: {
        ...
        employee_id: record.employee_id, company_id: record.company_id,
        role, must_change_pin: mustChangePin,
      },
    });
  } catch (err) {
...
router.post("/logout-all", async (req, res) => {
  ...
  try {
    ...
    await pool.query("UPDATE public.refresh_tokens SET revoked = TRUE WHERE user_id = $1", [payload.user_id]);
...
router.post("/signup", (req, res) => {
  return res.status(410).json({ ok: false, error: "SIGNUP_DISABLED", message: "Direct signup is disabled. Please use the invitation link sent to your email." });
});
router.post("/signup-invite", (req, res) => {
  return res.status(410).json({ ok: false, error: "SIGNUP_DISABLED", message: "This signup method is deprecated. Please use the invitation link sent to your email." });
});
  ...
  try {
    ...
    if (userId) {
      try {
        ...
        const row = q.rows?.[0] || {};
        return res.json({ ok: true, user: { ...payload, role: normalizeRole(row.role || payload.role), profile_status: row.profile_status || null } });
      } catch {
...
router.post("/change-pin", async (req, res) => {
  ...
  try {
    ...
    if (!current_pin || !new_pin) return res.status(400).json({ ok: false, error: "MISSING_FIELDS" });

    const newPinStr = String(new_pin);
    const role = payload.role ? String(payload.role).toUpperCase() : null;

    if (!isValidPin(newPinStr, role)) return res.status(400).json({ ok: false, error: "INVALID_PIN_FORMAT", message: "PIN must be 4-8 characters" });
    if (String(current_pin) === newPinStr) return res.status(400).json({ ok: false, error: "SAME_PIN", message: "New PIN must be different from current PIN" });

    const { rows } = await pool.query("SELECT id, pin_hash, must_change_pin FROM public.app_users WHERE id = $1 LIMIT 1", [String(payload.user_id)]);
    ...
    await pool.query(`UPDATE public.app_users SET pin_hash = $1, must_change_pin = false, is_temp_pin = false WHERE id = $2`, [newHash, String(payload.user_id)]);

    await audit(pool, req, { action: ACTIONS.PIN_CHANGED, entity_type: "user", entity_id: payload.user_id, entity_name: payload.username });
