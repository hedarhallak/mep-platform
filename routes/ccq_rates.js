...
router.post("/", async (req, res) => {
  try {
    const { trade_code, sector, min_km, rate_cad, tax_form, effective_from, effective_to, notes } = req.body || {};
    ...
    const { rows } = await pool.query(`
      ...
      RETURNING *
    `, [
      trade_code.toUpperCase(), sector.toUpperCase(),
      parseFloat(min_km), parseFloat(rate_cad),
      tax_form || null, effective_from, effective_to,
      notes || null, req.user?.user_id || null,
    ]);
...
router.patch("/:id", async (req, res) => {
  try {
    ...
    if (rate_cad != null)      { params.push(parseFloat(rate_cad));  fields.push(`rate_cad = $${params.length}`); }
    if (tax_form !== undefined) { params.push(tax_form || null);      fields.push(`tax_form = $${params.length}`); }
    if (effective_from)        { params.push(effective_from);        fields.push(`effective_from = $${params.length}`); }
    if (effective_to)          { params.push(effective_to);          fields.push(`effective_to = $${params.length}`); }
...
router.delete("/:id", async (req, res) => {
  try {
    const { rowCount } = await pool.query(
      `DELETE FROM public.ccq_travel_rates WHERE id = $1`,
