...
router.post('/', async (req, res) => {
  try {
    ...
    const rank_code = pickFirstNonEmpty(body, [
      'rank_code',
      'rank',
      'rankCode',
      'rank_name',
    ...
    const home_unit = pickFirstNonEmpty(body, [
      'home_unit',
      'unit',
      'apt',
      'apartment',
    ...
    if (e && e.code === '23505') {
      ...
      if (
        constraint === 'employee_profiles_phone_digits_uniq' ||
        constraint.includes('phone_digits')
      ) {
        return res.status(400).json({
          ok: false,
