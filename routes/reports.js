...
async function ccqZoneFromDB(pool, km, tradeCode, sector, dateStr) {
  ...
  try {
    ...
    if (!rows.length) {
      ...
      if (!fallback.length) return { zone: null, rate: 0, label: "No rate configured", tax_form: null };
...
function ccqZone(km, tradeCode, sector, dateStr) {
  ...
  const distKm = parseFloat(km || 0);
  if (distKm < 41) return { zone: null, rate: 0, label: "Not eligible (<41 km)", tax_form: null };
  if (distKm < 65) return { zone: "T2200/TP-64.3", rate: 0, label: `${distKm.toFixed(1)} km — T2200 + TP-64.3 (tax declaration)`, tax_form: "T2200 + TP-64.3" };
  return { zone: `${distKm.toFixed(1)}km`, rate: 0, label: "Use ccqZoneFromDB for rates", tax_form: null };
}
  ...
  try {
    ...
    if (isSelfOnly) {
      ...
      if (!emp.length || !emp[0].employee_id)
        return res.json({ ok: true, from, to, records: [], totals: { days_worked: 0, total_regular: 0, total_overtime: 0, total_hours: 0 } });
...
router.get("/travel", canAny(["reports.view", "reports.view_self"]), async (req, res) => {
  try {
    ...
    const enriched = await Promise.all(rows.map(async r => {
      const { zone, rate, label, tax_form } = await ccqZoneFromDB(pool, r.distance_km, r.trade_code, r.ccq_sector, r.first_date);
...
router.get("/my-daily", canAny(["reports.view", "reports.view_self"]), async (req, res) => {
  try {
    ...
    const enriched = await Promise.all(rows.map(async r => {
      const { zone, rate, label, tax_form } = await ccqZoneFromDB(pool, r.distance_km, r.trade_code, r.ccq_sector, r.attendance_date);
    ...
    const totals = {
      ...
      total_regular:   parseFloat(enriched.reduce((s, r) => s + parseFloat(r.regular_hours  || 0), 0).toFixed(2)),
      total_overtime:  parseFloat(enriched.reduce((s, r) => s + parseFloat(r.overtime_hours || 0), 0).toFixed(2)),
