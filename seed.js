...
const HOME_LOCATIONS = [
  ...
  { lat: 45.50, lng: -73.65, dist: 8.4 },
  { lat: 45.54, lng: -73.70, dist: 9.2 },
  { lat: 45.47, lng: -73.58, dist: 12.0 },
  { lat: 45.60, lng: -73.50, dist: 15.3 },
  { lat: 45.44, lng: -73.72, dist: 18.7 },
  { lat: 45.63, lng: -73.45, dist: 22.1 },
  { lat: 45.42, lng: -73.80, dist: 28.5 },
  { lat: 45.65, lng: -73.38, dist: 35.0 },
  // 41-65km — T2200
  { lat: 45.68, lng: -73.10, dist: 44.2 },
  { lat: 45.30, lng: -73.55, dist: 47.8 },
  { lat: 45.72, lng: -73.05, dist: 52.0 },
  { lat: 45.25, lng: -73.60, dist: 55.3 },
  { lat: 45.75, lng: -72.98, dist: 58.1 },
  { lat: 45.20, lng: -73.65, dist: 61.4 },
  // 65km+ — company allowance
  { lat: 45.80, lng: -72.85, dist: 68.5 },
  { lat: 45.15, lng: -73.70, dist: 72.1 },
  { lat: 45.85, lng: -72.72, dist: 78.3 },
  { lat: 45.10, lng: -73.75, dist: 82.0 },
  { lat: 45.90, lng: -72.60, dist: 88.7 },
  { lat: 45.05, lng: -73.80, dist: 92.4 },
  { lat: 45.95, lng: -72.48, dist: 97.2 },
  { lat: 45.00, lng: -73.85, dist: 103.5 },
  { lat: 46.00, lng: -72.35, dist: 110.8 },
];

const FIRST_NAMES = ["Mohammed","Jean","Pierre","Karim","Ali","David","Marc","Youssef","Antoine","Hassan","Lucas","Ethan","Noah","Liam","Omar","Carlos","Felipe","Andre","Michel","Paul"];
const LAST_NAMES  = ["Tremblay","Gagnon","Roy","Côté","Bouchard","Gauthier","Morin","Lavoie","Fortin","Gagné","Ouellet","Pelletier","Leblanc","Jobin","Ndiaye","Khalil","Hassan","Martin","Dupont","Bernard"];
  ...
  try {
    ...
    for (let i = 0; i < 50; i++) {
      ...
      const empRow = await client.query(`
        ...
        RETURNING id
      `, [`SEED-${String(i + 1).padStart(3, '0')}`, firstName, `${lastName} ${i + 1}`, COMPANY_ID, loc.lat, loc.lng]);

      const employeeId = empRow.rows[0].id;

      // Link app_user to employee
      await client.query(
        `UPDATE public.app_users SET employee_id = $1 WHERE id = $2`,
        [employeeId, userId]
      );
    ...
    for (const asgn of assignmentIds) {
      ...
      for (const day of selectedDays) {
        ...
        `, [
          COMPANY_ID, asgn.projectId, asgn.arId, asgn.employeeId,
          dateStr(day), checkIn, checkOut,
          regularHours, overtime, lateMinutes, status,
        ]);
        attendanceCount++;
      }
    }
    ...
    console.log(`   41-65km: ${HOME_LOCATIONS.filter(l => l.dist >= 41 && l.dist < 65).length * 2} employees`);
