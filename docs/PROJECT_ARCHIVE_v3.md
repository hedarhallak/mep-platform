...
## 12) ملاحظات جديدة مهمة حول Assignments (مرونة السوق + الصلاحيات + الاستيراد)

### 12.1 سيناريوهات التشغيل
Assignments يجب أن تدعم نموذجين بدون تغيير جذري:
1) **Centralized Dispatch** (مدير موارد/مدير مشاريع عام).
2) **Decentralized Dispatch** (PM/Foreman لكل مشروع).
ويجب دعم الانتقال التدريجي بينهما.

### 12.2 الصلاحيات RBAC
- **ADMIN**: كل شيء
- **DISPATCHER / OPS_MANAGER**: Create/Update/Delete لكل المشاريع + إدارة Requests
- **PM / FOREMAN**: ضمن مشاريع محددة + إرسال Requests + Approve/Reject حسب السياسة
- **WORKER**: وصول محدود (إن لزم)

### 12.3 واجهة مرنة (طرق عرض)
- **By Employee**: قائمة موظفين + أزرار: Assign / Modify / Request Modify / History
- **By Project**: قائمة مشاريع فعالة + موظفين + أزرار سريعة
كلاهما فوق نفس منطق overlap.

### 12.4 Geo-Assignment (لاحقًا)
Click/Drag employee → drop على project (Board UI) فوق نفس المنطق.
لا تُنفذ قبل تثبيت RBAC + Requests.

### 12.5 استيراد المشاريع والموظفين (قاعدة ثابتة)
لا إضافة يدوية للمشاريع أو الموظفين.
يجب دعم Import (CSV/Excel/ERP لاحقًا) للجداول:
- Projects
- Employees
...
## 13) تثبيت دور PROJECTS_ADMIN (قرار نهائي)

تم اعتماد تسمية **PROJECTS_ADMIN** كدور مركزي لإدارة التعيينات.

### الهدف من الدور
- إدارة التعيينات مركزيًا (Centralized Dispatch)
- الإشراف على Requests Inbox (Approve / Reject)
- تفويض الصلاحيات لاحقًا إلى PMs / Foremen بدون تغيير معماري

### مصفوفة الأدوار المعتمدة (Phase 1)
- **ADMIN**: صلاحيات كاملة على النظام
- **PROJECTS_ADMIN**: إدارة جميع Assignments + Requests
- **PM (Project Manager)**: إدارة Assignments ضمن مشاريع محددة + إرسال Requests
- **FOREMAN**: صلاحيات محدودة ضمن المشروع (حسب السياسة)
- **WORKER**: بدون صلاحيات على Assignments

> هذا القرار مغلق (LOCKED) ولا يُعاد فتحه إلا في Phase 2.
